function newtonFractal(x, y) {
  ...
  while (iterations < maxIterations) {
    ...
    // Compute f(z) = z^6 - 1
    // Compute f(z) = z^5 - z^3 + z
    let z3 = zx2 * zx - 3 * zx * zy2;
    let z4 = zx2 * zx2 - 6 * zx2 * zy2 + zy2 * zy2;
    let fzRe = z4 * zx - z3 + zx;
    let fzIm = z4 * zy - z3 * zy + zy;

    // Compute f'(z) = 5z^4 - 3z^2 + 1
    let fprimezRe = 5 * z4 - 3 * zx2 + 1;
    let fprimezIm = 5 * z4 * zy - 3 * zy2;
