function newtonFractal(x, y) {
  ...
  while (iterations < maxIterations) {
    ...
    // Compute f(z) = z^4 - z^2 + 1
    let fzRe = zx2 * zx2 - 6 * zx2 * zy2 + zy2 * zy2 - zx2 + 1;
    let fzIm = 4 * zx * zy * (zx2 - zy2) - 2 * zy;

    // Compute f'(z) = 4z^3 - 2z
    let fprimezRe = 4 * (zx2 * zx - 3 * zx * zy2) - 2 * zx;
    let fprimezIm = 4 * (3 * zx2 * zy - zy2 * zy) - 2 * zy;
